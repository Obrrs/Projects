...
function displayVerticalList(escolas, containerId) {
   ...
   if (!escolas || escolas.length === 0) {
       container.innerHTML = '<p class="text-center text-muted">Nenhuma Escola Profissional encontrada.</p>';
       return;
   }
   ...
   escolas.forEach(escola => {
       ...
       div.innerHTML = `
           ...
           <p class="small mb-1"><strong>Website:</strong> ${escola.website ? `<a href="${escola.website}" target="_blank" rel="noopener noreferrer">${escola.website}</a>` : 'N/A'}</p>
...
document.addEventListener('DOMContentLoaded', () => {
   console.log("INFO: DOM escolasProfissionais.html carregado.");
   // ...chama a função para carregar as escolas no div correto.
   loadEscolasPorTipo('Profissional', 'escolas-profissionais-list');
